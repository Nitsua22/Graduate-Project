...
app.all("/password", requireLogin, function (req, res) {
    ...
    if (Object.keys(req.body).length === 0) {
        ...
        // SCP: input sanitization using express sanitizer
        user[0] = req.sanitize(req.body.username);
        user[1] = req.sanitize(req.body.currentPassword);
        user[2] = req.sanitize(req.body.newPassword);

        var query1 = "USE users; SELECT username, password, id FROM appusers WHERE username=?";

        // construct sql statement
        // SCP: using prepared statements to construct queries which provides input sanitization
        query1 = mysql.format(query1, [user[0]]);
        ...
        conn.query(query1, (err, result) => {
            ...
            if (err) {
                errorMessage = "Account retrieval failed";
                ...
                if (result[1].length === 1) {
                    ...
                    bcrypt.compare(user[1], hash, (err2, isMatch) => {
                        // error with compare
                        if (err2) {
                            console.log(err2);
                        ...
                        // if username/password match
                        if (isMatch) {
                            // perform hashing on new password
                            bcrypt.hash(user[2], 10, function (err, hash) {
                                ...
                                if (err) {
                                    ...
                                    // construct query to register account
                                    //UPDATE APPUSERS SET password='changed' WHERE username='admin';
                                    var query2 = "USE users; UPDATE APPUSERS SET password=? WHERE username=?;"

                                    // SCP: using prepared statements to construct queries which provides input sanitization
                                    query2 = mysql.format(query2, [hash, user[0]]);
...
// SCP: using requireAdminOnly as a middleware
app.get('/appconfig', requireLogin, requireAdminOnly, function (req, res) {
...
//SCP: role based security implementation
function requireAdminOnly(req, res, next) {

    var userIsAdmin = false;

    // construct query
    var query = "USE users; SELECT role_id FROM appusers where username=?";

    // format query with session id
    query = mysql.format(query, req.session.username);
    ...
    conn.query(query, (err, result) => {
        ...
        // check if account is role_id 0 (admin)
        userIsAdmin = account['role_id'] === 0;
        ...
        if (userIsAdmin) {
            ...
            console.log("non admin user trying to access admin!");
            res.redirect('/home');
        }
    });
