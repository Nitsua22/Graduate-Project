...
// express setup and file upload setup
...
app.use('/main', serveIndex(path.join(__dirname, '')));
app.use('/main', express.static(path.join(__dirname, '')));
app.use('/main/profile_pictures', serveIndex(path.join(__dirname, 'profile_pictures')));
app.use('/main/profile_pictures', express.static(path.join(__dirname, 'profile_pictures')));
...
app.get('/viewprofile', requireLogin, checkIdWithUser, function (req, res) {
    ...
    // check whether the client had the userId in the url
    ...
    conn.query(queryAcct, function (err, result) {
        if (err) {
            ...
            if (result[1].length > 0) {
                ...
                userDetails['picture'] = "/main/profile_pictures/" + result[1][0]['picture'];

                // check whether the client requested receipts
                if (!req.query['date_from'] || !req.query['user_id']) {
                    ...
                    var query = "USE shopdb; SELECT * FROM receipts WHERE appuser_id = ? AND purchase_date >= ? ";

                    query = mysql.format(query, [userId, dateFrom])
...
function checkIdWithUser(req, res, next) {
    // if the session userID == the query userID, then allow the query
    if (req.session.userId && req.query['user_id']) {
        // if vulnerable, next
        if (vulnerable) {
            next();
        } else {
    ...
    if (Object.keys(req.body).length === 0) {
        ...
        bcrypt.hash(user[1], 10, function (err, hash) {
            ...
            if (err) {
                ...
                var query = "USE shopdb; INSERT INTO appusers (username, password, firstname, lastname, address, picture) VALUES (?, ?, ?, ?, ?, ?)";
                query = query.replace("?, ?, ?, ?, ?, ?", "'" + user[0] + "', '" + hash + "', '" + user[2] + "', '" + user[3] + "', '" + user[4] + "', '" + user[5] + "'");
...
// File uploading
app.post("/upload", requireLogin, checkIdWithUser, function(req, res) {
    let uploadFile;
    let uploadPath;
    ...
    user[0] = req.session.username;

    console.log('req.files >>>', req.files); // eslint-disable-line

    uploadFile = req.files.uploadFile;

    uploadPath = __dirname + '/profile_pictures/' + uploadFile.name;
    ...
    uploadFile.mv(uploadPath, function(err) {
        ...
        //UPDATE APPUSERS SET picture='changed' WHERE username='admin';
        var query = "USE shopdb; UPDATE APPUSERS SET picture=? WHERE username=?;"

        // SCP: using prepared statements to construct queries which provides input sanitization
        query = mysql.format(query, [uploadFile.name, user[0]]);

        conn.query(query, (err2, result) => {

            if (err2) {
                console.log(err2);
                errorMessage = "Profile picture change failed";

                res.render('login', {
                    _errorMessage: errorMessage
                });
            } else {
                console.log("Profile picture change complete!");
