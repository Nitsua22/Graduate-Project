/*
    3.3.2.2	Shopping App
        The shopping application will be modified in the following ways to expose the vulnerability:
        1.	All input captured from the client side shall not be sanitized before being used in queries
        2.	Error messages shall be sent to the client as-is
        3.	Default accounts will be set up for 'admin' 'admin' 
        4.	A new page for item sellers will be added

        The objectives of the student shall be to use this vulnerability to accomplish:
        1.	Simulate a credential stuffing attack on the application to gain access to an account

        # credential_stuffing_attack.ps1
...
app.all("/login", function (req, res) {
    ...
    if (Object.keys(req.body).length === 0) {
        ...
        var query = "USE shopdb; SELECT username, password, firstname, id FROM appusers WHERE username='" + userName + "'";
        //        var query = "USE shopdb; SELECT username, password, firstname, id FROM appusers WHERE username=?";
            ...
            if (err) {
                ...
                conn.query(query, (err2, result) => {
                    ...
                    if (err2) {
                        ...
                        req.session.accountName = user[2];
                        res.redirect('/home');
                    }
                });
            }
