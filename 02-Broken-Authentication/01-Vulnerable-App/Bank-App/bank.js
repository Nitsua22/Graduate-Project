...
3.3.2.1	 Bank App
    The bank application will be modified in the following ways to expose the vulnerability:
        1.	Pasword hashing/salting shall not be used when creating accounts
        2.	Sessionid will now be exposed in a get request parameter
        3.	Session timeout now 30 minutes

    The objectives of the student shall be to use this vulnerability to accomplish:
        1.	Using SQL Injection, get a list of all users usernames and passwords, then, withdraw all of the money from each account
        2.	Using a session value, log in to another user’s bank account and perform a transfer
            - this could be the session of a user that logged in to the machine before you
            - this could be the usertoken 
            
            
    Log in to the application
    Copy the url to go home
    Log out of the application
    paste the url to go home
...
app.get('/home', requireLogin, function (req, res) {
    ...
    var query;
    query = "USE users; SELECT * FROM accounts LEFT JOIN appusers ON accounts.appuser_id=appusers.id WHERE appusers.id=?";
    query = mysql.format(query, [sessionCache[req.query['sessionId']]]);
...
app.post('/transfer', function (req, res) {
    ...
    query = mysql.format(query, [accountSender, accountReceiver, req.sessino.username]);
...
/// updated to be vulnerable by having session id in browser
