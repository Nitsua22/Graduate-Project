/*
    ...
    3.3.4.2	Shopping App
        The shopping application will be modified in the following ways to expose the vulnerability:
        1.	A new feature shall be added to place a bulk order. The bulk order shall be formatted as an xml document
        2.	An xml parsing engine shall be implemented, with external entities enabled
*/


// whether this version of the app has the vulnerabilities on

const libxmljs = require("libxmljs"); // For parsing XML
const fs = require('fs'); // For file system operations
const fileUpload = require("express-fileupload"); // For file uploads
const {
    spawn
} = require('child_process'); // for creating the python process
...
app.get('/additem', function (req, res) {
    ...
    var id = req.query['id'];
    var item = {};
    var query = "USE shopdb; SELECT * FROM products WHERE id=" + id;
...
app.post("/upload", requireLogin, function (req, res) {
    // Make sure that the file was submitted
    if (!req.files) {
    ...
    let file = req.files.orders;

    let newFileName = file.tempFilePath + ".xml";

    fs.renameSync(file.tempFilePath, newFileName);

    var xmlStringParsed;

    // call the python process
    // execute python script
    const python = spawn('python', ['processOrder.py', newFileName]);

    // get data from python script stdout stream
    python.stdout.on('data', function (data) {
        xmlStringParsed = data.toString();

    });

    // after the python script has closed, send the data
    python.on('close', (code) => {

        // Parse the XML document
        let xmlDoc = libxmljs.parseXmlString(xmlStringParsed);

        // Get all XML nodes containing the appropriate 
        // information
        let xmlName = xmlDoc.get('/order/name');
        let xmlQuantity = xmlDoc.get('/order/quantity');
        let xmlAddress = xmlDoc.get('/order/address');
        let xmlPhone = xmlDoc.get('/order/phone');
        // id will be used to lookup item
        let xmlId = xmlDoc.get('/order/id');

        console.log("name: " + xmlName.text());
        console.log("q: " + xmlQuantity.text());
        console.log("a: " + xmlAddress.text());
        console.log("p: " + xmlPhone.text());
        console.log("id: " + xmlId.text());

        var quantity = xmlQuantity.text();
        var id = xmlId.text();

        // add the items to the cart

        // search for item in DB
        var query = "USE shopdb; SELECT * FROM products WHERE id=" + xmlId.text();

        query = mysql.format(query, xmlId.text());
        ...
        conn.query(query, function (err, result) {
            ...
            if (err) {
                ...
                itemFromDb['id'] = result[1][0]['id'];
                itemFromDb['name'] = xmlName.text();
                itemFromDb['price'] = result[1][0]['price'];
                itemFromDb['quantity'] = quantity;
...
app.all("/login", function (req, res) {
    ...
    if (Object.keys(req.body).length === 0) {
        ...
        var match;
        var query = "USE shopdb; SELECT username, password, id FROM appusers WHERE username='" + userName + "'";
...
app.all("/register", function (req, res) {
    ...
    if (Object.keys(req.body).length === 0) {
        ...
        // escape the user submitted datalocal
