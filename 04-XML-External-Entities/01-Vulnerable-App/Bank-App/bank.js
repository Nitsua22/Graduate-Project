/*
3.3.4	XML External Entities
    3.3.4.1	Bank App
        The bank application will be modified in the following ways to expose the vulnerability:
        1.	The feature to create a new account shall be modified to parse an xml document input from the client-side for account details
        2.	An xml parsing engine shall be implemented, with external entities enabled
        3.	A new directory shall be created and placed within the project, to simulate a typical file system directory that would be accessible to the application
...
const fileUpload = require("express-fileupload"); // For file 
const {
    spawn
        ...
        if (err) {
            ...
            if (result[1].length > 0) {
                ...
                result[1].forEach(function (acct) {
                    ...
                    account['accountLabel'] = acct['label'];
...
app.post("/upload", requireLogin, function (req, res) {
    ...
    const python = spawn('python', ['processAccountApplication.py', newFileName]);
    ...
    python.on('close', (code) => {
        ...
        let xmlDeposit = parseFloat(xmlDoc.get('/application/deposit').text());
        let xmlLabel = xmlDoc.get('/application/label').text(); // to comply with database length

        console.log("deposit: " + xmlDeposit);
        console.log("label: " + xmlLabel)
        ...
        query = "USE users; INSERT INTO accounts(amount, label, appuser_id) VALUES(?,?,(select id from appusers where username=?));";

        query = mysql.format(query, [xmlDeposit, xmlLabel, username]);
        ...
        conn.query(query, (err, result) => {
            ...
            if (err) {
                ...
                res.render('home', {
                    _accounts: req.session.accounts,
                    _errorMessage: "sql error! check logs",
                    _accountName: req.session.username
                });
                return;
            }
