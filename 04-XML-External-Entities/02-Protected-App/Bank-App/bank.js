...
app.post("/upload", requireLogin, function (req, res) {
    ...
    // check whether there are any external entities
    fs.readFile(file.tempFilePath, 'utf-8', function (err, data) {

        try {
            let testXml = libxmljs.parseXml(data, {
                noent: true
            });
            ...
            res.render('home', {
                _accounts: req.session.accounts,
                _errorMessage: "xml data has illegal configuration",
                _accountName: req.session.username
            });
            return;
        }
