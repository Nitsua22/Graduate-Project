...
mitigating the threat of man - in -the - middle attacks on the otherwise plaintext message
2. Hashing passwords
...
b.The shopping app checkout page shall be updated to capture the account details that were captured in the registration page, but shall only use them
for the purposes of generating a receipt, and shall not be stored in the database afterward.
...
app.post('/withdraw', requireLogin, function (req, res) {
    ...
    if (vulnerable) {
        ...
        amount = parseFloat(req.body.withdrawamount);
    }
...
app.post('/transfer', requireLogin, function (req, res) {
    ...
    query = mysql.format(query, [accontSender, accountReciever, req.session.username]);
